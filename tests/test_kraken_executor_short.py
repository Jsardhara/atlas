"""Verify Kraken executor handles SHORT direction correctly."""
...
from trader.kraken_executor import (
    KrakenExecutor,
    SHORT_MIN_LEVERAGE,
...
class _FakeKraken:
    def __init__(self):
        self.place_order = AsyncMock(return_value={"txid": ["KRAKEN-FAKE-1"]})
...
def patched_db(monkeypatch):
    """Stub get_session — kraken_executor uses it for size_position + persist."""
    from trader import kraken_executor as ke
    ...
    monkeypatch.setattr(ke, "get_session", _factory)


async def test_execute_short_uses_sell_side(fake_kraken, settings, patched_db):
    executor = KrakenExecutor(fake_kraken, settings)
    signal = {
        "pair": "XBT/USD",
        "direction": "SHORT",
        "confidence": 0.75,
    ...
    fake_kraken.place_order.assert_awaited_once()
    kwargs = fake_kraken.place_order.await_args.kwargs
...
async def test_execute_long_cash_uses_leverage_one(fake_kraken, settings, patched_db):
    executor = KrakenExecutor(fake_kraken, settings)
    signal = {
        "pair": "XBT/USD",
        "direction": "LONG",
...
async def test_execute_short_rejected_when_not_shortable(fake_kraken, settings, patched_db):
    executor = KrakenExecutor(fake_kraken, settings)
    signal = {
        "pair": "ADA/USD",
        "direction": "SHORT",
    ...
    result = await executor.execute_trade(signal, sizing, shortable_set={"XBTUSD"})
    assert result.get("rejected") is True
    fake_kraken.place_order.assert_not_called()


async def test_size_position_short_min_leverage(monkeypatch, fake_kraken, settings, patched_db):
    executor = KrakenExecutor(fake_kraken, settings)
    signal = {
        "pair": "XBT/USD",
        "direction": "SHORT",
...
async def test_size_position_long_high_confidence(monkeypatch, fake_kraken, settings, patched_db):
    executor = KrakenExecutor(fake_kraken, settings)
    signal = {"pair": "XBT/USD", "direction": "LONG", "confidence": 0.85}
    sizing = await executor.size_position(signal)
